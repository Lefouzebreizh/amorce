...
| `/demarrer-projet-client` | Fabriquer un projet client complet à partir du socle `agence/` en une commande vérifiée : copie, renommage, les trois branchements de la racine, workflow d'intégration continue. |
| `/supabase-en-direct` | Piloter un projet Supabase depuis la session quand le serveur MCP répond — schéma en migration, politiques éprouvées, advisors, clés — et quoi faire quand il ne répond pas. |
