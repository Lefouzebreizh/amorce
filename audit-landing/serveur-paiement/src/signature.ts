 ...
 *
 * Identique à `licence-serveur/src/signature.ts` — la vérification est
 * générique, pas propre à ce qu'elle protège, donc recopiée plutôt
 * qu'importée : deux projets npm distincts sous ce dépôt ne s'importent pas
 * l'un l'autre (même contrainte que la charte d'Artisan Express, voir
 * `titan-builder/CLAUDE.md`). Toute correction faite ici mérite d'être
...
/** Au-delà, on refuse : une signature valide rejouée plus tard ne doit rien déclencher. */
 ...
 * temps écoulé raconte alors combien de caractères étaient justes. On forge
 * la signature octet par octet à partir de là. La boucle ci-dessous parcourt
 * donc toujours toute la longueur.
 */
