...
//
// Copié depuis bilan-patrimoine/src/constats.ts (lot 1) pour le lot 2.
...
import type { Constat, Situation } from './modeles'
import { euros, montant, partPct, pourcent, reserve, valoriser } from './valorisation'
 ...
 * c'est-à-dire quand l'argent **perd** de la valeur. Sinon, l'argent ne perd
 * rien, mais reste disponible demain matin pour un besoin lointain — un
 * décalage qui mérite d'être vu, sans montant : chiffrer supposerait promettre
 * un rendement, ce que cet outil ne fait jamais.
 */
...
/**
 * Ce qui passe avant l'argent.
 *
 * Une réserve trop mince est un **risque**, et un risque passe avant une
 * optimisation, quel que soit le montant de celle-ci — un tri par euros seul
 * placerait « droit au LEP, 60 € par an » devant « votre matelas de sécurité
 * couvre moins de deux mois », ce qui contredirait le constat sur la réserve
 * lui-même. D'où ce rang, qui prime sur le montant. Il reste volontairement
 * minuscule : une seule urgence, et tout le reste se trie par ce qu'il coûte.
 */
