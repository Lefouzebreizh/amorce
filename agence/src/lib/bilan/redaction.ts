...
//
// Copié depuis bilan-patrimoine/src/redaction.ts (lot 1) pour le lot 2.
// `texte` (le rendu Markdown) est conservé pour rester fidèle à la source ;
// le site du lot 2 rend `constats`/`patrimoine` en cartes plutôt que ce texte.

import { VERIFIE_LE, baremesPerimes, tableAVerifier } from './baremes'
import { ETIQUETTES, type Constat, type Patrimoine, type Situation } from './modeles'
import { constater } from './constats'
import { euros, valoriser } from './valorisation'
...
/**
 * Le premier geste, et un seul — la phrase nue, sans habillage Markdown.
 *
 ...
 *
 * Exportée séparément de `premierGeste()` (qui l'enrobe en Markdown pour
 * `texte`) pour que le site du lot 2, qui rend des cartes structurées plutôt
 * que ce texte, puisse afficher la même phrase sans avoir à en retirer le
 * Markdown.
 */
export function premierGesteTexte(constats: readonly Constat[]): string | null {
  ...
  if (choisi === undefined) return null
  return GESTES[choisi.cle] ?? null
}

function premierGeste(constats: readonly Constat[]): string {
  const choisis = constats.filter((constat) => constat.ton !== 'bravo')
  const geste = premierGesteTexte(choisis)
