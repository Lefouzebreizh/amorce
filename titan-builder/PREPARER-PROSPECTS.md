...
Les exclusions rapprochent nom, identifiant, téléphone ou courriel normalisés. Une opposition, un envoi antérieur ou `inactive: true` bloque aussi les doublons présents dans le lot. Les téléphones sont rapprochés sous leur forme normalisée alphanumérique : conserver une convention commune (les formes `+33` et `0` ne sont pas encore unifiées). Toute source manquante empêche la sélection. Téléphone public sourcé et prestations sont requis pour générer une démo ; sinon le message propose de préparer un aperçu, sans prétendre qu'il existe déjà.
