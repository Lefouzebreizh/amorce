...
| API Gemini | **Joignable, et c'est le premier service d'IA tiers à l'être.** `generativelanguage.googleapis.com` rend **404 sur `/` et 403 sur `/v1beta/models`** — de vraies réponses HTTP, avec le JSON « Please use API Key ». Le tunnel, TLS et le routage fonctionnent : **une clé y servira réellement**, contrairement à ElevenLabs et Hedra. Mesuré le 01/09/2026, témoin `api.github.com` à 200. En revanche `aistudio.google.com` et `ai.google.dev` rendent 000 : la console et la documentation ne sont pas lisibles d'ici, seule l'API l'est. SDK officiel `google-genai` 2.21.0, installable depuis PyPI. **Il lit deux variables et `GOOGLE_API_KEY` a priorité sur `GEMINI_API_KEY`** (`_api_client.py:130`) : n'en poser qu'une. |
| Connecteur MCP ElevenLabs | **Il génère, et c'est ce qui a levé l'impossibilité de l'image.** Une illustration produite depuis une session distante le 01/09/2026 : 1344 × 768, `gemini-2.5-flash-image`, **4,57 centimes, dix secondes**. Porte 22 modèles d'image, 47 de vidéo, le lipsync, les bruitages, la musique, la voix et Scribe. **Le trafic d'un connecteur MCP ne passe pas par la liste de domaines** — l'API directe `api.elevenlabs.io` reste refusée au tunnel le même jour, pendant que le connecteur travaille. **Piège : le rapport sort en 16:9**, réglable seulement sur le nœud du flux, jamais à l'appel. `estimate_only: true` chiffre une génération sans rien dépenser — le faire avant tout lot. |
| Intermittence des connecteurs MCP | **Un connecteur absent de la boîte à outils n'est pas un connecteur perdu.** Le 01/09/2026, ElevenLabs et neuf autres serveurs se sont déconnectés puis reconnectés **quatre fois** dans la même séance, sans rien à faire. Une session qui trouve `ToolSearch` muet sur `mcp__…` ne doit donc ni conclure à une suppression, ni faire reconfigurer quoi que ce soit : elle le dit, elle avance sur ce qui n'en dépend pas, et elle réessaie plus tard. |
